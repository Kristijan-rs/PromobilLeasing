// src/components/BrandStrip.tsx
const LOGOS = [
  { src: "/brands/audi.svg", alt: "Audi" },
  { src: "/brands/bmw.svg", alt: "BMW" },
  { src: "/brands/mercedes.svg", alt: "Mercedes-Benz" }, // fajl stavi kako želiš
  { src: "/brands/volkswagen.svg", alt: "Volkswagen" },  // ili /brands/vw.svg
  { src: "/brands/tesla.svg", alt: "Tesla" },
  { src: "/brands/porsche.svg", alt: "Porsche" },
];

function Row() {
  return (
    <>
      {LOGOS.map((logo) => (
        <div
          key={logo.alt}
          className="inline-flex items-center justify-center px-3 opacity-80 hover:opacity-100 transition"
        >
          <img
            src={logo.src}
            alt={logo.alt}
            loading="lazy"
            decoding="async"
            className="h-10 w-auto"
          />
        </div>
      ))}
    </>
  );
}

export default function BrandStrip() {
  return (
    <section
      aria-label="Unsere Marken"
      className="relative overflow-hidden rounded-2xl  bg-white shadow-md"
    >
      {/* Fade ivice */}
      <div className="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-white to-transparent" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-white to-transparent" />

      {/* Track širine ~200%: dve iste serije zaredom */}
      <div className="relative py-6">
        <div
          className="
            whitespace-nowrap
            will-change-transform
            animate-[marquee_28s_linear_infinite]
          "
          // važno: nema padding/margina koje bi ubacile „gap“
        >
          {/* 1. polovina */}
          <div className="inline-flex items-center gap-10 align-middle">
            <Row />
          </div>
          {/* 2. polovina (identična), aria-hidden da ne duplira čitač ekrana */}
          <div className="inline-flex items-center gap-10 align-middle" aria-hidden="true">
            <Row />
          </div>
        </div>
      </div>

      {/* Keyframes lokalno (može i u index.css) */}
      <style>{`
        @keyframes marquee {
          0%   { transform: translateX(0); }
          100% { transform: translateX(-50%); }
        }
      `}</style>
    </section>
  );
}
